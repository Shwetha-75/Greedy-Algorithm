...
time of 
all the processes by following shortest job first policy.
...
'''
class Solution:
      def findShortestJobFirst(self,nums:list[int])->int:
          nums.sort()
          total,n=0,len(nums)
          sum_total=0
          for i in range(n):
...
class TestApp:
      def testing_case_one(self):
          assert Solution().findShortestJobFirst([4,3,7,1,2])==4 
      def testing_case_two(self):
          assert Solution().findShortestJobFirst([1,2,3,4])==2
